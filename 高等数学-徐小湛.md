...
(2) 重要极限 $\lim\limits_{x\to\infty}(1+\frac{1}{x})^x = \mathbb e$， $1^\infty$型

`幂指函数`: $f(x) = (1+\frac{1}{x})^x$

数列的极限 $\lim\limits_{n\to\infty}(1+\frac{1}{n})^n = \mathbb e$

例题：求 $\lim\limits_{x\to 0}(1+x)^{\frac{1}{x}}$

> 解： 令 $x=\frac{1}{t}$， 则 $t = \frac{1}{x}$，当 $x\to 0$ 时，$t \to \infty$
>
> $\lim\limits_{x\to 0}(1+x)^{\frac{1}{x}} = \lim\limits_{t\to\infty}(1+\frac{1}{t})^t = \mathbb e$

> 一般情况： $\lim\alpha = 0\quad \Rightarrow \quad \lim(1+\alpha)\frac{1}{\alpha} = \mathbb e$
...
> 
> $= \lim\limits_{x\to\infty}[(1+\frac{1}{\frac{x}{2}})^\frac{x}{2}]^2 = [\lim\limits_{x\to\infty}(1+\frac{1}{\frac{x}{2}})^\frac{x}{2}]^2 = \mathbb e^2$

`一般有以下重要公式`:

$\lim\limits_{x\to\infty}(1+\frac{k}{x})^x = \mathbb e^k \quad \quad \lim\limits_{x\to\infty}(1+\frac{k}{x})^{lx} = \mathbb e^{lk}$

$\lim\limits_{x\to 0}(1+kx)^\frac{1}{x} = \mathbb e^k \quad \quad \lim\limits_{x\to 0}(1+kx)^\frac{l}{x} = \mathbb e^{kl}$